...
<!--
  rundoc src:
  https://github.com/heroku/tenji-team/blob/master/deploying-spring-boot-apps.md
...
>note
>You can also start with a working [sample app](https://github.com/heroku/tenji-team) if you'd prefer.
...
The application does not have any custom logic by default -- it's just an empty template. To add some behavior, open the `src/main/java/com/tenji/demo/DemoApplication.java` file and put the following code in it:

```java
:::>> file.write src/main/java/com/tenji/demo/DemoApplication.java
package com.tenji.demo;
