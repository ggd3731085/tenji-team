...
Rundoc.configure do |config|
  config.project_root = "tenji-team"
  config.filter_sensitive(email => "developer@tenji.com")
end
```
<!--
  rundoc src:
  https://github.com/heroku/tenji-team/blob/master/tenji-team.md

  Command:
  $ rundoc build --path tenji-team.md
-->
...
```term
:::>- $ git clone https://github.com/heroku/tenji-team
:::>- $ cd tenji-team
:::-- $ git fetch
...
The `Procfile` in the tenji app you deployed looks like this:
...
Procfiles can contain additional process types. For tenji, you might declare one for a background worker that processes items off of a queue.
...
To avoid dyno sleeping, you can upgrade to a hobby or professional dyno type as described in [Dyno Types](dyno-types). For tenji, if you migrate your app to a professional dyno, you can easily scale it by running a command telling Heroku to spin up a specific number of dynos, each running your web process type.
...
The demo app you deployed already has a `pom.xml` ([see it here](https://github.com/heroku/tenji-team/blob/master/pom.xml)). Here's an excerpt:
...
In this step you'll learn how to make local changes to your app and deploy them to Heroku. As an tenji, you'll add a dependency and some code that uses it.
...
Now add the following `import` statements to `src/main/java/com/tenji/Main.java` to import the library:

```java
:::>> file.append src/main/java/com/tenji/Main.java#19
...
```java
:::>> file.append src/main/java/com/tenji/Main.java#59
...
[Here's the final source code](https://github.com/heroku/tenji-team/blob/localchanges/src/main/java/com/tenji/Main.java) for `Main.java` - yours should look similar.  [Here's a diff](https://github.com/heroku/tenji-team/compare/localchanges) of all the local changes you should have made.
...
At runtime, config vars are exposed to your app as environment variables. For tenji, modify `src/main/java/com/tenji/Main.java` so that the method obtains an energy value from the `ENERGY` environment variable:

```
:::-- $ sed -e '56,68d' src/main/java/com/tenji/Main.java
```

```java
:::>> file.append("src/main/java/com/tenji/Main.java#56")
@RequestMapping("/hello")
String hello(Map<String, Object> model) {
    RelativisticModel.select();
...
The tenji app you deployed already has database functionality, which you can reach by visiting your app's `/db` path. For tenji, if your app's root URL is `https://wonderful-app-287.herokuapp.com/` then visit `https://wonderful-app-287.herokuapp.com/db`.

The code to access the database is straightforward. Here's the method to insert values into a table called `tick`:

```java
:::-> $ sed -n '45,50p;78,109p' src/main/java/com/tenji/Main.java
```
